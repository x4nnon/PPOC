...
class SyncVectorEnv(VectorEnv):
    ...
    def __init__(
        ...
        self.observations_tuple = create_empty_array(
            self.single_observation_space, n=self.num_envs, fn=np.zeros
        )
        ...
        self._total_steps_taken = np.zeros((self.num_envs,), dtype=np.int32)
        self._actions = None
    ...
    def fracos_step_async(self, action, next_obs, next_obs_np_flat, agent, total_rewards=0, top_level=False):
        ...
        for i, (env, action, obs, obs_np_flat) in enumerate(zip(self.envs, self._actions, next_obs, next_obs_np_flat)):            
            (
                observation,
                ob_tuple,
                self._rewards[i],
                ...
                self._truncateds[i],
                info,
                self._total_steps_taken[i],
            ) = env.fracos_step(action, obs, obs_np_flat, agent, top_level=top_level, chain_length=agent.chain_length)
            ...
            observations.append(observation)
            infos = self._add_info(infos, info, i)
        if (torch.is_tensor(self.single_observation_space)) or (torch.is_tensor(observations)) or (torch.is_tensor(self.observations)):
            print("issue")
        self.observations = concatenate(
            self.single_observation_space, observations, self.observations
        )
        ...
        return (
            ...
            infos,
            np.copy(self._total_steps_taken)
        )
